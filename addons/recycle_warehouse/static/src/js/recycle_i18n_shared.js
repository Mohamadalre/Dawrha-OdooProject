...
reg('Receive shipment — scan or enter its QR code', 'استلام شحنة — امسح أو أدخل رمز QR');
reg('Receive Shipment', 'استلام الشحنة');
reg('Receiving…', 'جارٍ الاستلام…');
reg('Shipment received: ', 'تم استلام الشحنة: ');
reg('Scan or enter the shipment code first.', 'امسح أو أدخل رمز الشحنة أولاً.');
reg('Could not receive this shipment.', 'تعذّر استلام هذه الشحنة.');
...
reg('Decide This Split Order', 'قرار الطلبية المقسّمة');
reg('This is part of a split buyer order — you decide all its parts. Approving releases it to the output employees; rejecting cancels this part.',
    'هذا جزء من طلبية مشترٍ مقسّمة — أنت تقرّر جميع أجزائها. الموافقة تُتيحه لموظفي الإخراج؛ الرفض يُلغي هذا الجزء.');
...
reg('This phone number is already registered for another employee.', 'رقم الهاتف هذا مسجل بالفعل لموظف آخر.');
reg('Applying…', 'جارٍ التطبيق…');
reg('Assigned Quantity', 'الكمية المخصّصة');
reg('Could not apply the change.', 'تعذّر تطبيق التغيير.');
reg('Could not load suggestions.', 'تعذّر تحميل الاقتراحات.');
reg('Current', 'الحالي');
reg('Distance (km)', 'المسافة (كم)');
reg('Loading suggestions…', 'جارٍ تحميل الاقتراحات…');
reg('Modify Split — Algorithm Suggestions', 'تعديل التقسيم — اقتراحات الخوارزمية');
reg('No alternative set of the same size covers this order right now.', 'لا توجد مجموعة بديلة بنفس الحجم تغطّي هذا الطلب حالياً.');
reg('Order rejected and cancelled.', 'تم رفض الطلب وإلغاؤه.');
reg('Received here, but the collection system was not updated. Tell your manager.', 'تم الاستلام هنا، لكن نظام الجمع لم يُحدَّث. أبلغ مديرك.');
reg('Search by product…', 'البحث بالمنتج…');
reg('Show suggestions', 'عرض الاقتراحات');
reg('Split', 'تقسيم');
reg('Split Across Warehouses', 'التقسيم عبر المستودعات');
reg('Split re-routed to the chosen warehouses.', 'أُعيد توجيه التقسيم إلى المستودعات المختارة.');
reg('Suggested Warehouses', 'المستودعات المقترحة');
reg('Swap this split onto a different set of warehouses that together cover the whole order — the same number of warehouses, nearest first. The allocator suggests the sets.', 'بدّل هذا التقسيم إلى مجموعة مستودعات أخرى تغطّي معاً كامل الطلب — بنفس عدد المستودعات، الأقرب أولاً. يقترح المُخصِّص المجموعات.');
reg('This buyer order was divided across these warehouses. Each row is the quantity assigned to that warehouse. The whole split is decided by the administrator as ONE order — not per warehouse.', 'تم تقسيم طلب المشتري هذا عبر هذه المستودعات. كل صف هو الكمية المخصّصة لذلك المستودع. ويقرّر المسؤول التقسيم بأكمله كطلب واحد — لا لكل مستودع.');
reg('This order has no backend reference to modify.', 'لا يملك هذا الطلب مرجعاً في الباك يند لتعديله.');
reg('Use this set', 'استخدم هذه المجموعة');
reg('warehouses', 'مستودعات');
